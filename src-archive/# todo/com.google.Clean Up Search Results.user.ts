...
function get(selector) {
  return new Promise(async (resolve) => {
    watchForDescendants({ selector }, (element) => {
      resolve(element);
      return false;
    });
  });
...
function processSearchResultRoot(rso, root) {
  ...
  watchForDescendants({ selector: SELECTOR.SEARCH_RESULT_SECTION_TITLE, root }, (child) => {
  ...
  watchForDescendants({ selector: SELECTOR.SEARCH_RESULT_ITEM, root }, (child) => {
...
function catchSectionsToIgnore(root) {
  watchForDescendants({ selector: SELECTOR.SEARCH_RESULT_SKIPPABLE, root }, () => {
    hide(root);
    return false;
  });
...
get(SELECTOR.SEARCH_RESULT_SECTION).then((rso) => {
  watchForChildren({ selector: '*', root: rso }, (child) => {
...
function processBotStuffRoot(botstuff, root) {
  hide(root);
  watchForDescendants({ selector: SELECTOR.NAVIGATION, root }, (child) => {
...
get(SELECTOR.BOTTOM_SECTION).then((botstuff) => {
  watchForChildren({ selector: '*', root: botstuff }, (child) => {
