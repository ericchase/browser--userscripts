// ==UserScript==
// @name        youtube.com: Remove Unwanted Videos
// @author      ericchase
// @namespace   ericchase
...
function parseAge(ageString) {
  ...
  const count = Number.parseInt(ageString[0]);
...
function toSeconds(timeString) {
  ...
  if (timePieces === 2) return Number.parseInt(timeString[0] * 60) + Number.parseInt(timeString[1]);

  return Number.parseInt(timeString[0] * 3600) + Number.parseInt(timeString[1] * 60) + Number.parseInt(timeString[2]);
}
