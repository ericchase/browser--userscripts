...
function preload() {
  ...
  Watch = class {
    static childList({ node, subtree, callback }) {
      new MutationObserver(callback).observe(node, { childList: true, subtree: subtree });
    }
...
function main() {
  ...
  waitfor([tabbar_selector, morelist_selector], reorder_tabs);
}
...
function waitfor(selectors, callback) {
  ...
  function tryadd(item, timer) {
    ...
    clearInterval(timer);
    item_list.push(item);
    if (item_list.length == selectors.length) callback(item_list);
  }

  let timers = [];
  selectors.forEach((selector) => {
    timers.push(setInterval((selector, id) => tryadd(document.querySelector(selector), timers[id]), 10, selector, timers.length));
  });
}
...
function reorder_tabs([tabbar, morelist]) {
  ...
  Array.from(taborder).forEach((name) => {
    let newtab;
    if (name === active_tabname) newtab = active_structure.cloneNode(true);
    else newtab = inactive_structure.cloneNode(true);
    settext(newtab, name);
  ...
  Array.from(infomap).forEach((entry) => {
    let name = entry[0];
    let href = entry[1];
    let newtab;
    if (name === active_tabname) newtab = active_structure.cloneNode(true);
    else newtab = extra_structure.cloneNode(true);
    settext(newtab, name);
    sethref(newtab, href);
    if (name === active_tabname) tabbar.appendChild(newtab);
    else morelist.appendChild(newtab);
  });
...
function get_tab_structure_active(tabs) {
  return strip_tab(tabs.find((tab) => tab.classList.contains('hdtb-msel')).cloneNode(true));
}
function get_tab_structure_inactive(tabs) {
  return strip_tab(tabs.find((tab) => tab.classList.contains('hdtb-msel') === false).cloneNode(true));
}
...
function deltext(tab) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) tab.childNodes[0].innerText = '';
  else tab.innerText = '';
}
function gettext(tab) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) return tab.childNodes[0].innerText;
  else return tab.innerText;
}
function settext(tab, value) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) tab.childNodes[0].innerText = value;
  else tab.innerText = value;
}

function delhref(tab) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) tab.childNodes[0].setAttribute('href', '');
  else tab.setAttribute('href', '');
}
function gethref(tab) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) return tab.childNodes[0].getAttribute('href');
  else return tab.getAttribute('href');
}
function sethref(tab, value) {
  if (tab.nodeName === 'DIV' && tab.classList.contains('hdtb-msel') === false) tab.childNodes[0].setAttribute('href', value);
  else tab.setAttribute('href', value);
}
