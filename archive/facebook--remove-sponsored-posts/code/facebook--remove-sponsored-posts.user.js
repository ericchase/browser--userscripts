...
function preload() {
  ...
  let Stream = class {
    ...
    static of(array) {
      ...
      for (let n = 0; n < array.length; ++n) newStream.elements.push(array[n]);
      return newStream;
    }
    ...
    anyMatch(predicate) {
      ...
      for (let n in list) if (predicate(list[n])) return true;
      return false;
    }
    ...
    filter(predicate) {
      ...
      for (let n in list) if (predicate(list[n])) filteredStream.elements.push(list[n]);
    ...
    forEach(consumer) {
      let list = this.elements;
      for (let n in list) consumer(list[n]);
    }
  };
...
function lookfor_feed(node) {
  var node_iterator = document.createNodeIterator(node, NodeFilter.SHOW_ELEMENT, (descendant) => {
    if (descendant.getAttribute('role') === 'feed') return NodeFilter.FILTER_ACCEPT;
  });
...
function lookfor_posts(node) {
  var node_iterator = document.createNodeIterator(node, NodeFilter.SHOW_ELEMENT, (descendant) => {
    if (descendant.getAttribute('data-testid') === 'fbfeed_story') return NodeFilter.FILTER_ACCEPT;
  });
  ...
  for (let i = 0; i < document.styleSheets.length; i++) {
    ...
    if (rules)
      for (let x = 0; x < rules.length; x++) {
        ...
        if (sponsored_class_list.length === 3) return;
      }
  }
}
...
function remove_sponsored_posts(node) {
  if (sponsored_class_list.length !== 3) get_sponsored_class_list();

  if (lookfor_sponsored(node)) remove(node);
}

function lookfor_sponsored(node) {
  var node_iterator = document.createNodeIterator(node, NodeFilter.SHOW_ELEMENT, (descendant) => {
    if (Array.from(sponsored_class_list).some(bind(descendant.classList, 'contains'))) return NodeFilter.FILTER_ACCEPT;
  });
  return node_iterator.nextNode();
}
