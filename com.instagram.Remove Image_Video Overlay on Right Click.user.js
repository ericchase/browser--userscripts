// ==UserScript==
// @name        instagram.com: Remove Image/Video Overlay on Right Click
// @author      ericchase
// @namespace   ericchase
...
function removeChildren(elParent, elIgnore) {
  const elRemovalList = [];
  for (const elChild of elParent.children) if (elChild !== elIgnore && !elChild.contains(elIgnore)) elRemovalList.push(elChild);
  for (const el of elRemovalList) el.remove();
}
