...
async function addDatabaseOperations(list, item, url) {
  ...
  if (item.firstElementChild) {
    ...
    const div = createElement('div', {
      classList: ['mr-2', 'd-flex', 'flex-items-center'],
    });
    div.append(raiseA, lowerA);
    item.firstElementChild.prepend(div);

    // Delete
    const deleteA = createElement('a', {
      classList: ['ml-1'],
      textContent: '⨯',
    });
    deleteA.href = '#';
...
function createFavoritesSection() {
  ...
  const title = createElement('h2', {
    classList: ['f5'],
    textContent: 'Favorite Repositories',
  });
...
function createRepositoryListItem(userName, repoName) {
  ...
  const containerDiv = createElement('div', {
    classList: ['width-full', 'd-flex', 'mt-2'],
  });
  listItem.appendChild(containerDiv);
  const pictureA = createElement('a', {
    classList: ['mr-2', 'd-flex', 'flex-items-center'],
  });
